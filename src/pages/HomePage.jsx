...
const HomePage = () => {
  ...
  const fetchAllcourses = async () => {
    try {
      const response = await axios.get(
        'https://lms-backend-ol4a.onrender.comcourses/fetch/status-wise?status=active'
      );
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await axios.get(
          'https://lms-backend-ol4a.onrender.comcategory/fetch/all?status=active'
        );
  ...
  const searchByCourseName = async () => {
    ...
    if (trimmedName !== '') {
      try {
        const response = await axios.get(
          `https://lms-backend-ol4a.onrender.comcourses/fetch/name-wise?courseName=${trimmedName}&status=active`
        );
        setCourses(response.data.data);
      } catch (error) {
        console.log(error);
  ...
  useEffect(() => {
    const fetchCategoryCourses = async () => {
      if (category !== 0) {
        try {
          const response = await axios.get(
            `https://lms-backend-ol4a.onrender.comcourses/fetch/category-wise?categoryId=${category}&status=ACTIVE`
          );
